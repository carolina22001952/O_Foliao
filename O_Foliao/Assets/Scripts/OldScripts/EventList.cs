...
public class EventList : MonoBehaviour
{
    ...
    [SerializeField]
    private const int highAlcoholLvl = 80;
    ...
    private List<Events> alcoholEvents = null;
    ...
    [SerializeField] private List<Events> lowAlcoholDeck;
    [SerializeField] private List<Events> highAlcoholDeck;
    ...
    {
        int alcool = player.GetAlcohol();
        if (alcool <= lowAlcoolLvl)
        {
            return lowAlcoholDeck;
        }
        else if (alcool >= highAlcoholLvl)
        {
            return highAlcoholDeck;
        }
        else
        {
            return null;
    ...
    /*public Events ChooseRandomEvent(Player player, Node node, EventsUi eventsUi)
    {
        ...
        //if tempor�rio apenas arranjo r�pido para uma feature
        if (events == hotelDeck)
        {
            return hotelDeck[0];
        }
            alcoholEvents = ChooseAlcoolEvents(player);
